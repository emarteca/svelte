...
var TIMING_TEMP_VAR_AUTOGEN18__RANDOM = perf_hooks.performance.now();
 await  query(`
			insert into gists(uid, user_id, name, files)
			values ($1, $2, $3, $4)
		`, [req.params.id, user.id, data.description, JSON.stringify(files)]);
console.log("/home/ellen/Documents/ASJProj/TESTING_reordering/svelte/site/src/routes/repl/[id]/index.json.js& [45, 2; 48, 72]& TEMP_VAR_AUTOGEN18__RANDOM& " + (perf_hooks.performance.now() - TIMING_TEMP_VAR_AUTOGEN18__RANDOM));
...
var TIMING_TEMP_VAR_AUTOGEN21__RANDOM = perf_hooks.performance.now();
 var AWAIT_VAR_TIMING_TEMP_VAR_AUTOGEN21__RANDOM = await  query(`
		select g.*, u.uid as owner from gists g
		left join users u on g.user_id = u.id
		where g.uid = $1 limit 1
	`, [req.params.id]); 
console.log("/home/ellen/Documents/ASJProj/TESTING_reordering/svelte/site/src/routes/repl/[id]/index.json.js& [87, 1; 91, 21]& TEMP_VAR_AUTOGEN21__RANDOM& " + (perf_hooks.performance.now() - TIMING_TEMP_VAR_AUTOGEN21__RANDOM));
 const [row] =  AWAIT_VAR_TIMING_TEMP_VAR_AUTOGEN21__RANDOM
...
return send(res, 403, { error: 'Item does not belong to you' });
